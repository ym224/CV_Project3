...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    # project point to z=1 plane at (xt/zt, yt/zt, 1)
    _xn = xt / zt
    _yn = yt / zt

    # distort with radial distortion coeff
    r_squared = _xn ** 2 + _yn ** 2
    xt = _xn * (1 + k1 * r_squared + k2 * r_squared ** 2)
    yt = _yn * (1 + k1 * r_squared + k2 * r_squared ** 2)
